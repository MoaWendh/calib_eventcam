...
bool get_pipeline_configuration(int argc, char *argv[], RecorderConfig &config) {
    ...
    base_options.add_options()
        ...
        ("input-event-file,i",    po::value<std::string>(&config.event_file_path_), "Path to input event file (RAW or HDF5). If not specified, the camera live stream is used.")
        ("input-camera-config,j", po::value<std::string>(&config.cam_config_path_), "Path to a JSON file containing camera config settings to restore a camera state. Only works for live cameras.")
...
int calibration_recording(int argc, char *argv[]) {
    ...
    if (conf_.event_file_path_.empty()) {
        try {
            camera = Metavision::Camera::from_first_available();
            if (!conf_.cam_config_path_.empty()) {
                camera.load(conf_.cam_config_path_);
            }
        ...
        camera = Metavision::Camera::from_file(conf_.event_file_path_,
